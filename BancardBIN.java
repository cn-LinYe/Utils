...
public class BancardBIN {
     ...
     * @return
     */
    public static String getNameOfBank(String number6) {
        int bin = 0, index = 0;
        bin = Integer.valueOf(number6);
    ...
    public static void main(String[] args){
        boolean eq = bankName.length == bankBin.length;
        System.out.println(eq);
        Set<String> names = Sets.newHashSet(bankName);
        names = names.stream().map(name -> {
            String n = "";
            String[] bankinfos = StringUtils.split(name,".");
            if(bankinfos.length == 1){
                n = bankinfos[0];
            }else if(bankinfos.length == 2){
                n = bankinfos[0];
            }else{
                n = StringUtils.join(Arrays.stream(bankinfos).limit(bankinfos.length-1).collect(Collectors.toList()), ".");
            }
            return n;
